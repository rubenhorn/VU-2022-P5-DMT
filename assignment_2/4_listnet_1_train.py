...
model_out_path = (Path(__file__).parent / 'models' / f'{dataset_name}-ListNet').resolve()
...
def create_batches(data):
    ...
    for _, group in data.groupby('srch_id'):
        ...
        y = group.apply(lambda row: row['booking_bool'] * 5 + row['click_bool'] * 1, axis=1)
        X_batches.append(X)
        y_batches.append(y)
...
def create_model(docs_per_query, embedding_dims):
    ...
    scores_prob_dist = tf.keras.layers.Dense(units=docs_per_query, activation='softmax', name='scores_prob_dist')
